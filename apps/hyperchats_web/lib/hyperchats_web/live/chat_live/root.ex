defmodule HyperchatsWeb.ChatLive.Root do
  use HyperchatsWeb, :live_view
  alias Hyperchats.Chats
  alias HyperchatsWeb.Endpoint
  alias HyperchatsWeb.ChatLive.{Rooms, Room, Message}
  ...
  def handle_event("load_more", _params, socket) do
    messages = Chats.get_previous_n_messages(socket.assigns.oldest_message_id, 5)
  ...
  def insert_new_message(socket, message) do
    socket
    |> stream_insert(:messages, Chats.preload_message_sender(message))
  end

  def insert_updated_message(socket, message) do
    socket
    |> stream_insert(:messages, Chats.preload_message_sender(message), at: -1)
  end

  def assign_active_room_messages(socket) do
    messages = Chats.last_ten_messages_for(socket.assigns.room.id)
  ...
  def assign_rooms(socket) do
    assign(socket, :rooms, Chats.list_rooms())
  end

  def assign_active_room(socket, id) do
    assign(socket, :room, Chats.get_room!(id))
  end
  ...
  def assign_last_user_message(%{assigns: %{room: nil}} = socket) do
    assign(socket, :message, %Chats.Message{})
  end
  ...
  def delete_message(socket, message_id) do
    ...
    Chats.delete_message(message)
    stream_delete(socket, :messages, message)
  end

  def get_last_user_message_for_room(room_id, current_user_id) do
    Chats.last_user_message_for_room(room_id, current_user_id) || %Chats.Message{}
  end
end
